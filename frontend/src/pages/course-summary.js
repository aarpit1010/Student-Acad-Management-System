...
const CourseSummary = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      const getDroppedCourses = await axios.get("/student/droppedcourses", {
        headers: {
          "auth-token": localStorage.token,
          "Content-Type": "application/json",
        },
                ...
                data.courses.marks.map((c, courseKey) => {
                  return (
                    <tr key={courseKey}>
                      ...
                      <td>{c.course_ID}</td>
                      <td>{c.course_Name}</td>
                      <td>{data.courses.marks[courseKey].marks.c1}</td>
                      <td>{data.courses.marks[courseKey].marks.c2}</td>
                      <td>{data.courses.marks[courseKey].marks.c3}</td>
                      <td>{data.courses.marks[courseKey].marks.total}</td>
                      <td>{data.courses.marks[courseKey].marks.gpa}</td>
                    </tr>
                  );
                })}
            </tbody>
